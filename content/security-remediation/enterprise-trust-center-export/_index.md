...
{{< callout type="info" >}}
**What this is.** SecurityRecipes should be easy for an enterprise reviewer
...
| Section | Purpose |
| --- | --- |
| `export_summary` | Trust-center readiness, pack counts, section counts, failure counts, MCP tool count, readiness summary, threat radar summary, BOM summary, and trust review-readiness snapshot. |
| `pack_index` | Required evidence packs with paths, hashes, schemas, categories, failure counts, summaries, status, and MCP tools. |
| `trust_center_sections` | reviewer-readable control areas such as secure context, MCP control plane, agent identity, handoffs, assurance/evals, and market strategy. |
...
| `diligence_questions` | Specific reviewer questions with answers, evidence paths, and MCP tools to inspect next. |
...
| `vendorization_path` | Open, team, enterprise, and reviewer value paths without closing the public knowledge base. |
...
This is the review and diligence wrapper a trusted-source project needs. The
open corpus creates distribution. The generated packs create machine
readable trust. The MCP server turns those packs into an operational surface.
The trust-center export ties the pieces together so a reviewer can quickly
